...
🔴 **A trusted publisher has to be registered by a human at npmjs.com per package, all six registered and confirmed 2026-08-14, and proven on 2026-08-15 when `vitepress-dgmo` 0.6.5 became the first version Actions published — it carries a provenance attestation and 0.6.4 does not** — registration can only be read off each package's own settings page, never from a terminal, and an empty `npm view <pkg> dist.attestations` does **not** answer it (it proves only that nothing published from CI, which was true because releases were run locally — `@diagrammo/dgmo` turned out to be registered all along under that misreading). Package → Settings → Trusted Publisher → GitHub Actions, organization `diagrammo`, repository the package's own, workflow filename `release.yml`, environment blank, allowed action `npm publish`. The runbook at `diagrammo-ecosystem-docs/src/content/docs/infrastructure/npm-trusted-publishers.md` (live at https://docs.diagrammo.app/infrastructure/npm-trusted-publishers/) lists all ten with their exact field values; there is no API and it cannot be automated, so until it is done the publish step fails to **authenticate**. A non-empty `npm view <pkg> dist.attestations` is the proof it published from CI.

⚠️ **The wrappers declare `remark-dgmo` as an open `>=X <1` range, which never re-resolves on its own** — each wrapper's lockfile and its fixture's exact pins stay where they are. Landing a release here is not done until every wrapper's range floor and fixture pins are bumped; check with the `jq` sweep:
